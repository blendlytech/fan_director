...
| `migrations/0002_phase2.sql` | Adult switch per creator, stored boundary flags, `fan_restriction`, `safety_case_evidence` |
| `src/drafts.ts`, `src/validation.ts` | Owner-scoped DraftV2 save and reload, with validation, the rules layer and quotes |
| `src/catalog.ts`, `src/publish.ts` | Catalog read, quotes and staleness; publish checks and statements |
| `src/rules/`, `src/screening.ts` | The hard-list rules layer (rules-v1) and what a block does |
| `src/submission.ts` | `validateForSubmission`, with no route until Phase 4 |
| `../shared/` | Catalog types, validation, quotes, ranges and the boundaries renderer, shared with the frontend |
| `seeds/pilot-maya-v1.sql` | Maya's pilot catalog v1, generated by `scripts/publish-catalog.ts` |
...
| `test/` | 170 tests: real RS256 tokens and real local D1; only Clerk's Backend API is faked |
...
| GET | `/api/creators/:creatorId/drafts/:draftId/quote` | fan | The draft's quote under its own version, with `stale` and the new version's breakdown |
| POST | `/api/creators/:creatorId/drafts/:draftId/accept-catalog-version` | fan | `{ expectedRevision, catalogVersionId }`: moves the draft to the current version, or a typed error and no change |
...
ids or boundary flags. The catalog version must belong to that creator's
catalog (`422 catalog_version_mismatch`), and it can't change on update.
Bodies over 32 KB give `413`. The custom request is at most 1,000 characters.

Every save runs, in order (doc 11 §8 Phase 2):

1. **Selections** against the version, using `shared/domain/validate.ts`: selection
   groups, quantities, requires/excludes, hidden and adult gating, and the
   personalised-video resale rule. Failures are `422 selection_rejected` with a
   typed `reason`, or `422 personalised_video_resale_forbidden`. Nothing is repaired.
2. **Custom-request policy:** `decline` gives `422 custom_requests_not_accepted`.
3. **The hard-list rules layer** (`src/rules/`) over every text field, alone and together:
   `422 hard_list_blocked { key, lines, field }`. The audit event keeps only the key,
   a neutral subject, the layer and the ruleset version. The third block in 24 hours
   sets `fan_restriction` (AI off, flagged for review). A `minors` hit opens a
   `safety_case` with the full request in `safety_case_evidence`, suspends the fan,
   and answers `403 account_paused`.
4. **Creator limits:** a hard-no gives `422 creator_limit_blocked { limit, label, field }`;
   an ask-me becomes a stored `boundaryFlags` entry.

Responses carry the draft, `quote` (integer cents) and `stale`. When the catalog
has a newer version, `stale: true` comes with `current`: the new version's
breakdown, or the typed error it gives. The price never changes until the fan
calls `accept-catalog-version`; until then, edits get `409 catalog_version_stale`.

`validateForSubmission` (`src/submission.ts`) is a function only. There is no
submission endpoint until Phase 4.

### Publishing a catalog

There is no editor UI. `scripts/publish-catalog.ts` checks a version
(structure, adult content off, and the rules layer over every creator-written
text) and writes the SQL that publishes it and retires the previous version:

```sh
node scripts/publish-catalog.ts pilot --out seeds/pilot-maya-v1.sql
npx wrangler d1 execute DB --env staging --remote --file seeds/pilot-maya-v1.sql
```

`node scripts/rules-rates.ts` prints the rules layer's rates on the test sets.

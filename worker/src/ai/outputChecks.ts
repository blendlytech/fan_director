...
const DIGITS = '(?:\\d+|one|two|three|four|five|six|seven|eight|nine|ten|a couple of|a few)'
...
const CHECKS: Check[] = [
  ...
  // Delivery timing: durations in days, hours or weeks, dates and deadlines.
  {
    issue: 'delivery_time',
    pattern: new RegExp(
      `\\b${DIGITS}\\s*(?:-\\s*)?(?:business\\s+)?(?:days?|hours?|hrs?|weeks?|months?)\\b|\\b(?:tomorrow|tonight|next week|this week|end of the week|same[- ]day|overnight|asap|in no time)\\b|\\b(?:by|on|before|until)\\s+${WEEKDAYS}\\b|\\b${MONTHS}\\s+\\d{1,2}(?:st|nd|rd|th)?\\b|\\b\\d{1,2}(?:st|nd|rd|th)?\\s+(?:of\\s+)?${MONTHS}\\b|\\b\\d{1,2}/\\d{1,2}(?:/\\d{2,4})?\\b|\\b(?:deliver\\w*|arriv\\w*|shipp?\\w*|turnaround)\\b`,
      'i',
    ),
  ...
  // Approval: only the creator decides (doc 11 §3 rule 4).
  {
    issue: 'approval',
    pattern:
      /\b(?:approv\w*|guarantee\w*|confirm\w*|accepted|accepts|agreed|agrees|booked|booking|promise\w*|reserved|will definitely|for sure|it'?s a deal|signed off|green[- ]?light\w*|will (?:do|make|film|shoot|record|include|add) (?:it|this|that)|(?:she|he|they|maya)(?:'ll| will) (?:do|agree|say yes|love to|happily))\b|\bsaid yes\b/i,
  },
...
export function checkOutputText(field: string, text: string): OutputFinding[] {
